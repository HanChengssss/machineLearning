...
class navie_bayes_classifier:
    ...
    def __init__(self, data=datasets):
        ...
        self._priori_prob = cal_base_rates(self._data)  # 计算各种水果的先验概率
        self._likelihold_prob = likelihold_prob(self._data)  # 计算各种特征的后验概率
        self._evidence_prob = evidence_prob(self._data)  # 计算特征的先验概率
...
if __name__ == '__main__':
    a,b = count_total(datasets)
    # print(a, b)
    # cal_base_rates = cal_base_rates(datasets)
    likelihold = likelihold_prob(datasets)
    # evi = evidence_prob(datasets)
    n = navie_bayes_classifier()
    res = n.get_label(length='not_long', sweetness="sweet", color="yellow")

    # print(evi){'banala': 0.063, 'orange': 0.15, 'other_fruit': 0.018750000000000003}
    print(res)
